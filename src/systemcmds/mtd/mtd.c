...
static bool attached = false;  //绑定标志
static bool started = false;   //开始标志
...
int mtd_main(int argc, char *argv[])   //程序入口
{
    if (argc >= 2) {  //收到有用参数
		if (!strcmp(argv[1], "start")) {
			...
			if (argc >= 3) {
				...
				mtd_start(partition_names_default, n_partitions_default); //FRAM分为两个部分，参数和路径规划，将所有空间分半
			}
		}
...
ramtron_attach(void)
{
    ...
    struct spi_dev_s *spi = stm32_spibus_initialize(PX4_SPI_BUS_RAMTRON);  //spi2初始化

	/* this resets the spi bus, set correct bus speed again */
    SPI_SETFREQUENCY(spi, 10 * 1000 * 1000); //设置速度
	SPI_SETBITS(spi, 8);
    SPI_SETMODE(spi, SPIDEV_MODE3);  //spi模式
    SPI_SELECT(spi, SPIDEV_FLASH, false); //芯片选择，spi flash设备

	if (spi == NULL) {  //spi初始化失败
		errx(1, "failed to locate spi bus");
	}

	/* start the RAMTRON driver, attempt 5 times */
	for (int i = 0; i < 5; i++) {
		mtd_dev = ramtron_initialize(spi); //创建初始化mtd设备

		if (mtd_dev) {//设备初始化成功
			/* abort on first valid result */
			if (i > 0) {//如果尝试不止一次，给出警告
	...
	if (mtd_dev == NULL) {//mtd设备初始化失败
		errx(1, "failed to initialize mtd driver");
	}

    int ret = mtd_dev->ioctl(mtd_dev, MTDIOC_SETSPEED, (unsigned long)10 * 1000 * 1000); //设置设备速度
	...
	attached = true;  //设备绑定成功标志
}

static void
mtd_start(char *partition_names[], unsigned n_partitions) //启动mtd设备
{
	int ret;

	if (started) {  //如果启动标志已置位
		errx(1, "mtd already mounted");
	}

	if (!attached) { //绑定标志没有置位
        ramtron_attach();  //绑定
	}
	...
	unsigned long blocksize, erasesize, neraseblocks; //一次性读写块大小，一次性擦除块大小，总共可擦除块数量
	unsigned blkpererase, nblocks, partsize; //一次性擦除大小除以一次性读写大小，分成的部分，每个部分的大小
	...
	FAR struct mtd_dev_s *part[n_partitions]; //分成了两个部分，1参数，2路径规划
	char blockname[32];
	...
	for (offset = 0, i = 0; i < n_partitions; offset += nblocks, i++) {
		...
		part[i] = mtd_partition(mtd_dev, offset, nblocks); //mtd两个部分空间分配
		...
		ret = bchdev_register(blockname, partition_names[i], false);//对创建的块设备进行字符设备注册
		     ...
		     unsigned *blkpererase, unsigned *nblocks, unsigned *partsize, unsigned n_partitions)
{
    ...
    int ret = mtd_dev->ioctl(mtd_dev, MTDIOC_GEOMETRY, (unsigned long)((uintptr_t)&geo)); //获得存储器结构
    ...
    *blocksize = geo.blocksize;  //一次读写块大小
    *erasesize = geo.blocksize;   //一次擦除块大小
    *neraseblocks = geo.neraseblocks;  //可擦除块数量

	/* Determine the size of each partition.  Make each partition an even
	 * multiple of the erase block size (perhaps not using some space at the
     * end of the FLASH).确定每个部分的大小，使每个部分是多个可查出块大小的倍数
	 */

    *blkpererase = geo.erasesize / geo.blocksize;  //一次性可擦除大小初一一次读写大小
	*nblocks     = (geo.neraseblocks / n_partitions) * *blkpererase; //分成几个部分
    *partsize    = *nblocks * geo.blocksize; //部分的大小
