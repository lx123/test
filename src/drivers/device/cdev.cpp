...
static const unsigned pollset_increment = 0;//轮询设置，计数
//标准的nuttx操作分配表不能直接调用C++成员功能函数，我们不得不用一个跳板绑定这个分配表
static int	cdev_open(file_t *filp); //函数声明
...
/**
 * Character device indirection table.字符设备重定向表
 *
...
const struct file_operations CDev::fops = { //设备操作结构体
open	: cdev_open,
close	: cdev_close,
read	: cdev_read,
write	: cdev_write,
...
CDev::CDev(const char *name,//构造函数
	   const char *devname,//设备名称，一般为路径
	   int irq) ://中断号
	// base class
	Device(name, irq),//父类构造
	// public
	// protected
	_pub_blocked(false),
	// private
...
CDev::~CDev() //析构函数
{
	if (_registered) {//如果注册过，则注销设备
		unregister_driver(_devname);
	}
}

int
CDev::register_class_devname(const char *class_devname)  //注册类设备
{
...
CDev::init()
{
    ...
    int ret = Device::init();//基类首先初始化，注册中断
	...
	if (_devname != nullptr) {
        ret = register_driver(_devname, &fops, 0666, (void *)this);//注册驱动，系统函数
...
/*
 * Default implementations of the character device interface默认执行字符设备接口
 */
int
...
CDev::ioctl(file_t *filp, int cmd, unsigned long arg)
{
    ...
    /* fetch a pointer to the driver's private data */ //获取一个驱动私有成员的指针
	case DIOC_GETPRIV:
...
CDev::poll_notify(pollevent_t events)
{
    ...
    irqstate_t state = up_irq_save();//系统函数
...
int
CDev::remove_poll_waiter(struct pollfd *fds)//移除轮询等待者
{
	for (unsigned i = 0; i < _max_pollwaiters; i++) {
		if (fds == _pollset[i]) {
