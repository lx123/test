...
class MPU6000_gyro;  //陀螺仪类

class MPU6000 : public device::SPI  //传感器类，继承spi
{
public:
	...
	virtual ssize_t		read(struct file *filp, char *buffer, size_t buflen);  //读取
...
protected:
	...
	friend class MPU6000_gyro;  //友元类

	virtual ssize_t		gyro_read(struct file *filp, char *buffer, size_t buflen);  //陀螺仪读取
	virtual int		gyro_ioctl(struct file *filp, int cmd, unsigned long arg);      //陀螺仪io控制

private:
	int 			_device_type;    //设备类型
	MPU6000_gyro		*_gyro;   //陀螺仪类
	uint8_t			_product;	/** product code */

	struct hrt_call		_call;       //回调函数
	unsigned		_call_interval;  //回调间隔

	ringbuffer::RingBuffer	*_accel_reports;   //环形缓冲区

	struct accel_scale	_accel_scale;      //加速计缩放
	float			_accel_range_scale;     //加速计量程缩放
	float			_accel_range_m_s2;     //加速计量程 m/s2
	orb_advert_t		_accel_topic;       //加速计会话句柄
	int			_accel_orb_class_instance;  //加速计会话类编号
	int			_accel_class_instance;      //加速计类编号

	ringbuffer::RingBuffer	*_gyro_reports;  //陀螺仪缓冲区

	struct gyro_scale	_gyro_scale;          //陀螺仪缩放
	float			_gyro_range_scale;     //陀螺仪量程缩放
	float			_gyro_range_rad_s;     //陀螺仪量程 弧度/s

	unsigned		_sample_rate;       //采样速率
	...
	math::LowPassFilter2p	_accel_filter_x;   //2阶低通滤波
	...
	Integrator		_accel_int;       //加速计积分
	Integrator		_gyro_int;        //陀螺仪积分
...
#define MPU6000_NUM_CHECKED_REGISTERS 10
	...
	// self test
	volatile bool		_in_factory_test;  //标志位，在原厂测试中
	 ...
	 */
	static void		measure_trampoline(void *arg);  //测量跳板
	...
	/*
	  set low pass filter frequency  设置低通滤波频率
	 */
	void _set_dlpf_filter(uint16_t frequency_hz);

	/*
	  set sample rate (approximate) - 1kHz to 5Hz  设置采样频率
	*/
 ...
 */
class MPU6000_gyro : public device::CDev    //陀螺仪类，继承字符设备
{
public:
...
protected:
	friend class MPU6000;  //友元类

	void			parent_poll_notify();   //父类轮询通知

private:
	MPU6000			*_parent;
	orb_advert_t		_gyro_topic;    //陀螺仪主题
...
MPU6000::MPU6000(int bus, const char *path_accel, const char *path_gyro, spi_dev_e device, enum Rotation rotation,   //构造函数
		 int device_type) :
	SPI("MPU6000", path_accel, bus, device, SPIDEV_MODE3, MPU6000_LOW_BUS_SPEED),   //spi初始化
	_device_type(device_type),
	_gyro(new MPU6000_gyro(this, path_gyro)),  //陀螺仪类实例
	_product(0),
	_call{},
	_call_interval(0),
	...
	_got_duplicate(false)
{
	...
	_device_id.devid_s.devtype = DRV_ACC_DEVTYPE_MPU6000;  //设备类型
...
MPU6000::init()  //传感器初始化
{
	...
	_accel_reports = new ringbuffer::RingBuffer(2, sizeof(accel_report));   //创建环形缓冲区
	...
	_accel_class_instance = register_class_devname(ACCEL_BASE_DEVICE_PATH);   //加速计设备注册
	...
	/* measurement will have generated a report, publish */  //创建会话
	_accel_topic = orb_advertise_multi(ORB_ID(sensor_accel), &arp,
					   &_accel_orb_class_instance, (is_external()) ? ORB_PRIO_MAX : ORB_PRIO_HIGH);

	if (_accel_topic == nullptr) {  //会话创建失败
		warnx("ADVERT FAIL");
	}
...
int MPU6000::reset()  //设备复位
{
	...
	// come as zero
	uint8_t tries = 5;   //尝试5此

	while (--tries != 0) {
		irqstate_t state;
		state = up_irq_save();

		write_reg(MPUREG_PWR_MGMT_1, BIT_H_RESET);  //写入寄存器
		up_udelay(10000);
...
*/
void
MPU6000::_set_sample_rate(unsigned desired_sample_rate_hz)   //设置采样频率
{
 ...
 */
void
MPU6000::_set_dlpf_filter(uint16_t frequency_hz)  //设置低通滤波器
{
	uint8_t filter;
...
ssize_t
MPU6000::read(struct file *filp, char *buffer, size_t buflen)  //读取数据
{
...
uint8_t
MPU6000::read_reg(unsigned reg, uint32_t speed)  //读取寄存器
{
...
int
MPU6000::set_accel_range(unsigned max_g_in)   //设置加速计量程
{
...
MPU6000::measure()   //测量
{
	if (_in_factory_test) {   //如果在工厂检测，直接返回
	...
	*/
	...
	rotate_3f(_rotation, xraw_f, yraw_f, zraw_f);  //用户旋转，一般无旋转
	...
	if (gyro_notify && !(_pub_blocked)) {
		/* publish it */
		orb_publish(ORB_ID(sensor_gyro), _gyro->_gyro_topic, &grb);  //发布数据
	}
...
MPU6000_gyro::MPU6000_gyro(MPU6000 *parent, const char *path) :  //陀螺仪类
...
{
	int fd;
	MPU6000 **g_dev_ptr = external_bus ? &g_dev_ext : &g_dev_int;  //外部总线还是内部，实例化
	...
	if (*g_dev_ptr != nullptr)  //设备不为空，说明已经启动
...
#else
		spi_dev_e cs = (spi_dev_e) PX4_SPIDEV_MPU;  //设备片选编号
#endif
...
test(bool external_bus)
{
	...
	sz = read(fd, &a_report, sizeof(a_report));  //读取数据
...
mpu6000_main(int argc, char *argv[])
{
	...
	int device_type = 6000;  //设备编号为mpu6000
	int ch;
	enum Rotation rotation = ROTATION_NONE;  //传感器不旋转，默认
	int accel_range = 8;    //加速计量程8g
	...
	if (!strcmp(verb, "start")) {
		mpu6000::start(external_bus, rotation, accel_range, device_type); //内部总线，不旋转，加速计量程，设备mpu6000
	}
