...
io_timer_channel_allocation_t channel_allocations[IOTimerChanModeSize] = { UINT8_MAX,   0  ,  0   ,  0 };  //定时器通道调用
...
int io_timer_is_channel_free(unsigned channel)  //判断通道是不是空闲的
{
...
int io_timer_get_mode_channels(io_timer_channel_mode_t mode)  //定时器获取通道模式
{
	if (mode < IOTimerChanModeSize) {  //模式为有效模式
		return channel_allocations[mode];  //返回通道调用
	}
...
int io_timer_free_channel(unsigned channel)  //释放通道
{
	if (io_timer_validate_channel_index(channel) != 0) {  //先判断通道有效
		return -EINVAL;
	}
...
static int allocate_channel(unsigned channel, io_timer_channel_mode_t mode)
{
	...
	if (mode != IOTimerChanMode_NotUsed) {  //如果使用了通道
		rv = io_timer_validate_channel_index(channel);  //判断定时器有效通道
...
int io_timer_channel_init(unsigned channel, io_timer_channel_mode_t mode,
			  channel_handler_t channel_handler, void *context)   //通道号，通道模式，通道句柄，上下文
{
	...
	switch (mode) {
	case IOTimerChanMode_PWMOut:  //pwm输出
		ccer_setbits = 0;
		dier_setbits = 0;
	...
	int rv = allocate_channel(channel, mode);  //申请通道
	...
	if (rv >= 0) {
		...
		io_timer_init_timer(channels_timer(channel));  //初始化定时器
