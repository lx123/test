...
/* register addresses: A: accel, M: mag, T: temp */  //寄存器地址
...
/* default values for this device */  //这个设备默认的设置
...
private:
    ...
    struct hrt_call		_accel_call;   //回调函数
...
#define LSM303D_NUM_CHECKED_REGISTERS 8
	 ...
	 */
    bool			is_external() { return (_bus == EXTERNAL_BUS); }   //判断是外部总线还是内部总线
	 ...
	 */
    static void		measure_trampoline(void *arg);   //跳板
	 ...
	 * @param arg		Instance pointer for the driver that is polling.
	 */
    static void		mag_measure_trampoline(void *arg);  //地磁跳板

	/**
	 * check key registers for correct values
	 */
    void			check_registers(void);  //检查关键寄存器
	 ...
	 */
    int			accel_set_range(unsigned max_g);  //设置量程
	 ...
	 * @return		OK if the value can be supported, -ERANGE otherwise.
	 */
    int			mag_set_range(unsigned max_g);   //设置地磁量程
	 ...
	 * @return		OK if the value can be supported, -ERANGE otherwise.
	 */
    int			accel_set_onchip_lowpass_filter_bandwidth(unsigned bandwidth);  //设置低通滤波器贷款
	 ...
	 */
    int			accel_set_samplerate(unsigned frequency);   //设置采样频率
...
protected:
    friend class LSM303D;   //友元类

    void				parent_poll_notify();  //父类轮询通知
private:
	LSM303D				*_parent;
	...
	_checked_next(0)
{
    ...
    _device_id.devid_s.devtype = DRV_ACC_DEVTYPE_LSM303D;  //设备类型
...
LSM303D::~LSM303D()  //析构函数
{
	...
	if (_accel_class_instance != -1) {
        unregister_class_devname(ACCEL_BASE_DEVICE_PATH, _accel_class_instance);  //注销设备
	}
...
LSM303D::init()
{
    ...
    _accel_reports = new ringbuffer::RingBuffer(2, sizeof(accel_report));  //创建环形缓冲区，加速计

	if (_accel_reports == nullptr) {
		goto out;
	}

    _mag_reports = new ringbuffer::RingBuffer(2, sizeof(mag_report));  //环形缓冲区，地磁
    ...
    _accel_class_instance = register_class_devname(ACCEL_BASE_DEVICE_PATH);  //注册加速计设备
...
void
LSM303D::disable_i2c(void)  //禁止I2C
{
...
LSM303D::ioctl(struct file *filp, int cmd, unsigned long arg)
{
    ...
    case SENSORIOCSPOLLRATE: {  //S轮询速率
			switch (arg) {

			/* switching to manual polling */
            case SENSOR_POLLRATE_MANUAL:  //切换到手动轮询
				stop();
				_call_accel_interval = 0;
				return OK;
            ...
            case SENSOR_POLLRATE_MAX:  //设置最大轮询速率
...
LSM303D::accel_self_test()  //加速计自检
{
    ...
    if (fabsf(_accel_scale.x_offset) < 0.000001f) {  //检查加速计偏移
		return 1;
	}
...
uint8_t
LSM303D::read_reg(unsigned reg)  //读取寄存器
{
	uint8_t cmd[2];
...
int
LSM303D::mag_set_range(unsigned max_ga)  //设置地磁量程
{
	uint8_t setbits = 0;
...
int
LSM303D::accel_set_samplerate(unsigned frequency)  //设置采样速率
{
	uint8_t setbits = 0;
...
LSM303D::start()
{
    ...
    /* start polling at the specified rate */  //开始轮询
...
#pragma pack(pop)
    ...
    bool accel_notify = _accel_int.put(accel_report.timestamp, aval, aval_integrated, accel_report.integral_dt);  //积分
...
LSM303D_mag::init()
{
    ...
    _mag_class_instance = register_class_devname(MAG_BASE_DEVICE_PATH);  //注册设备
...
LSM303D_mag::measure()
{
    _parent->mag_measure();  //地磁测量
}

void
LSM303D_mag::measure_trampoline(void *arg)
{
    _parent->mag_measure_trampoline(arg);  //测量跳板
}
...
void	start(bool external_bus, enum Rotation rotation, unsigned range);   //开始
void	test();          //测试
void	reset();         //复位
void	info();          //信息
void	regdump();       //寄存器处理
void	usage();         //使用方法
void	test_error();    //测试错误

/**
 * Start the driver.  启动驱动器
 *
 * This function call only returns once the driver is
 * up and running or failed to detect the sensor.
 */
void
start(bool external_bus, enum Rotation rotation, unsigned range)  //启动
{
	int fd, fd_mag;
	...
	if (external_bus) {
        ...
        g_dev = new LSM303D(PX4_SPI_BUS_SENSORS, LSM303D_DEVICE_PATH_ACCEL, (spi_dev_e)PX4_SPIDEV_ACCEL_MAG, rotation);  //新建对象
	}
    ...
    if (OK != g_dev->init()) {   //设备初始化
		goto fail;
	}

	/* set the poll rate to default, starts automatic data collection */
    fd = open(LSM303D_DEVICE_PATH_ACCEL, O_RDONLY);  //设置轮询速率，开始自动数据采集

    if (fd < 0) {   //文件句柄为空，文件打开失败
		goto fail;
	}
    ...
    if (ioctl(fd, ACCELIOCSRANGE, range) < 0) {    //设置加速计量程
		goto fail;
	}

    fd_mag = open(LSM303D_DEVICE_PATH_MAG, O_RDONLY);   //打开地磁文件

	/* don't fail if open cannot be opened */
	if (0 <= fd_mag) {
        if (ioctl(fd_mag, SENSORIOCSPOLLRATE, SENSOR_POLLRATE_DEFAULT) < 0) {  //设置地磁轮询速率
			goto fail;
		}
	}
...
fail:  //失败
    ...
    errx(1, "driver start failed");  //驱动开始失败
}
...
test()   //测试
{
    int fd_accel = -1;   //加速计文件句柄
    struct accel_report accel_report;   //发布句柄
	ssize_t sz;
	int ret;
    ...
    warnx("accel range: %8.4f m/s^2", (double)accel_report.range_m_s2);  //加速计量程

    if (ERROR == (ret = ioctl(fd_accel, ACCELIOCGLOWPASS, 0))) {  //低通滤波
    ...
    int fd_mag = -1;   //文件句柄
    struct mag_report m_report;   //地磁发布结构体

	/* get the driver */
    fd_mag = open(LSM303D_DEVICE_PATH_MAG, O_RDONLY);  //打开设备

    if (fd_mag < 0) { //文件句柄小于零，文件打开失败
...
reset()   //复位驱动器
{
    int fd = open(LSM303D_DEVICE_PATH_ACCEL, O_RDONLY);  //打开文件

	if (fd < 0) {
		err(1, "failed ");
	}

    if (ioctl(fd, SENSORIOCRESET, 0) < 0) { //驱动复位
		err(1, "driver reset failed");
	}

    if (ioctl(fd, SENSORIOCSPOLLRATE, SENSOR_POLLRATE_DEFAULT) < 0) {  //设置默认的轮询速率
		err(1, "accel pollrate reset failed");
	}

    close(fd);  //关闭文件

    fd = open(LSM303D_DEVICE_PATH_MAG, O_RDONLY);  //打开地磁设备
...
info()  //打印一些关于驱动的信息
{
	...
	printf("state @ %p\n", g_dev);
    g_dev->print_info();  //打印信息

	exit(0);
}

/**
 * dump registers from device  导出设备寄存器信息
 */
void
regdump()
{
...
int
lsm303d_main(int argc, char *argv[])  //定义在匿名名字空间
{
    bool external_bus = false;  //外部总线标志
	int ch;
    enum Rotation rotation = ROTATION_NONE;  //芯片旋转标志
	int accel_range = 8;
