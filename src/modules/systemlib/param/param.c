...
#else
// FIXME - start and end are reversed  开始和结束失败
static const struct param_info_s *param_info_base = (const struct param_info_s *) &px4_parameters;  //关键参数，python生产
#endif

#define	param_info_count		px4_parameters.param_count

/**
 * Storage for modified parameters.  保存修改的参数
 */
struct param_wbuf_s {
	param_t			param;
...
static unsigned
get_param_info_count(void)  //获取参数信息计数
{
...
UT_array	*param_values;   //灵活的数组，保持修改参数的值

/** array info for the modified parameters array */  //数组信息，关于修改参数数组
const UT_icd	param_icd = {sizeof(struct param_wbuf_s), NULL, NULL, NULL};

/** parameter update topic */  //参数更新主题
ORB_DEFINE(parameter_update, struct parameter_update_s);  //主题定义

/** parameter update topic handle */  //参数更新会话句柄
static orb_advert_t param_topic = NULL;

static void param_set_used_internal(param_t param);  //参数内部设置

static param_t param_find_internal(const char *name, bool notification);  //内部参数查找
...
{
    int count = get_param_info_count();  //获取信息计数
 ...
 */
static int
param_compare_values(const void *a, const void *b)  //比较参数信息
{
...
param_find_changed(param_t param)  //查找参数变更
{
...
static void
param_notify_changes(bool is_saved)  //通知参数改变，发布到uorb
{
	 ...
	 */
	if (param_topic == NULL) {
        param_topic = orb_advertise(ORB_ID(parameter_update), &pup);  //发布参数更新主题

	} else {
        orb_publish(ORB_ID(parameter_update), param_topic, &pup);  //直接发布
	}
}

param_t
param_find_internal(const char *name, bool notification)  //查找内部参数
{
	param_t param;

	/* perform a linear search of the known parameters */

	for (param = 0; handle_in_range(param); param++) { //对数量进行判断
        if (!strcmp(param_info_base[param].name, name)) {  //对名字进行匹配
			if (notification) {
				param_set_used_internal(param);
			}

            return param;  //如果找到，返回
		}
	}

	/* not found */
    return PARAM_INVALID;  //没有找到参数
}

param_t
param_find(const char *name)  //参数查找
{
...
unsigned
param_count_used(void) //参数使用计数
{
	unsigned count = 0;

	// ensure the allocation has been done
    if (get_param_info_count()) {  //获取参数信息计数
...
param_t
param_for_index(unsigned index)  //参数
{
...
param_t
param_for_used_index(unsigned index)  //参数使用索引
{
    int count = get_param_info_count(); //获取参数数量信息
...
int
param_get_index(param_t param)  //参数获取索引
{
