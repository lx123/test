...
namespace uORB
{
class DeviceNode;     //设备节点类
class DeviceMaster;   //主设备类
}
...
public:
	 ...
	 */
	virtual int  open(struct file *filp); //方法。创建订阅者编号，返回指向订阅的文件的指针

	/**
	 * Method to close a subscriber for this topic.
	 */
	virtual int   close(struct file *filp);  //方法。关闭订阅者主题
	 ...
	 */
	virtual ssize_t  read(struct file *filp, char *buffer, size_t buflen);  //从提供的缓冲区节点中读取数据
	...
	/**
	 * Method to publish a data to this node. 向节点发布数据的方法
	 */
...
protected:
	virtual pollevent_t poll_state(struct file *filp);  //轮询状态
...
private: // private class methods.
	 ...
	 */
	void      update_deferred();  //执行一个延迟更新堆一个速率限制的订阅者
...
/**
 * Master control device for ObjDev.  主控制设备
 *
