...
/**
 * Maximum number of multi topic instances  最大主题实例数量
 */
 ...
 * a given topic. 给指定会话 产生一个指向uORB元数据结构的指针
 *
 * The topic must have been declared previously in scope
 * with ORB_DECLARE(). 会话必须已经在之前声明了，
 *
 * @param _name		The name of the topic.
 */
#define ORB_ID(_name)		&__orb_##_name  //元数据指针
...
/**
 * Define (instantiate) the uORB metadata for a topic.  定义uORB元数据对于一个会话
 *
 ...
 */
extern orb_advert_t orb_advertise(const struct orb_metadata *meta, const void *data) __EXPORT;  //无返回类型
 ...
 *			this function will return -1 and set errno to ENOENT.
 */
extern orb_advert_t orb_advertise_multi(const struct orb_metadata *meta, const void *data, int *instance,  //无返回类型
 ...
 * @return		OK on success, ERROR otherwise with ERRNO set accordingly.
 */
extern int	orb_set_interval(int handle, unsigned interval) __EXPORT;  //设置会话最小的时间间隔
