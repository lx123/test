...
/**
 * Advertise as the publisher of a topic.  //发布一个会话
 *
 ...
 */
orb_advert_t orb_advertise(const struct orb_metadata *meta, const void *data) //第一个参数为结构体，第二个为数据
{
	return uORB::Manager::get_instance()->orb_advertise(meta, data);  //get_instance()获得一个manager类
}
 ...
 *      this function will return -1 and set errno to ENOENT.
 */
orb_advert_t orb_advertise_multi(const struct orb_metadata *meta, const void *data, int *instance,  //多个主题
				 int priority)
{
...
/**
 * Publish new data to a topic.  向一个主题上发布新数据
 *
...
/**
 * Subscribe to a multi-instance of a topic.  订阅多个会话的主题
 *
 * The returned value is a file descriptor that can be passed to poll()
 * in order to wait for updates to a topic, as well as topic_read,
 * orb_check and orb_stat.
...
/**
 * Unsubscribe from a topic.  取消主题订阅
 *
 * @param handle  A handle returned from orb_subscribe.
 * @return    OK on success, ERROR otherwise with errno set accordingly.
 */
int  orb_unsubscribe(int handle) //取消订阅一个会话
{
...
/**
 * Check whether a topic has been published to since the last orb_copy. 检查是否有新数据发布到主题上
 *
...
/**
 * Return the last time that the topic was updated.  返回最后一次更新的时间
 *
 * @param handle  A handle returned from orb_subscribe.
...
/**
 * Check if a topic has already been created.  检查一个主题是否存在
 *
...
/**
 * Get the number of published instances of a topic group  获得主题组数量
 *
 * @param meta    ORB topic metadata.
...
/**
 * Return the priority of the topic  返回主题优先级
 *
 * @param handle  A handle returned from orb_subscribe.
